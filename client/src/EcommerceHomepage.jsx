    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-blue-800 text-white">
        <div className="max-w-[1400px] mx-auto px-4">
          <nav className="flex items-center gap-6 text-xs py-2">
            <span className="font-semibold">Mall</span>
            {['Mobiles', 'Electronics', 'Home & Kitchen', 'Furniture', 'Two Wheeler', 'Cars', 'Education', 'Health', 'Tractor'].map((item, idx) => (
              <button key={idx} className="hover:text-blue-200">{item}</button>
            ))}
          </nav>
        </div>
      </div>

      {/* Quick Categories - Compact Circles */}
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-2">
          {quickCategories.map((cat, idx) => (
            <div key={idx} className="flex flex-col items-center gap-1.5 cursor-pointer group flex-shrink-0">
              <div className={`w-15 h-15 p-3 bg-[#f5f5f5] rounded-full flex items-center justify-center text-2xl group-hover:scale-110 transition shadow-sm `}>
               
                <img className='w-5 ' src={phoneImage} alt="" />
              </div>
              <span className="text-[10px] text-gray-700 text-center leading-tight w-16">{cat.name}</span>
            </div>
          ))}
        </div>
      </section> */}
      ...
      <section className="max-w-[1400px] mx-auto px-4 pb-4">
        <div className="relative rounded-lg overflow-hidden bg-gradient-to-r from-orange-300 via-orange-200 to-orange-100 h-64 shadow-lg">
